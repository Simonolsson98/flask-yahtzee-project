...
def after_input():
    if request.method == "POST":
        ...
        return render_template("index.html", three_of_a_kind = f"{list_of_probs[0]*100}%", four_of_a_kind = f"{list_of_probs[1]*100}%", 
...
def calc_prob(die1, die2, die3, throwcount):
    if throwcount == "1":
        if(die1 == die2 and die2 == die3):
            ...
            return [round(3/6, 2), round(347/4997, 2), round(0.2222, 2), round(1/(6**3), 4)]
        return [-1,-1,-1,-1]
    else: #throwcount = 2
        if(die1 == die2 and die2 == die3):
            return [1, round(4/6, 2), round(2/6, 2), round(2/(6**2), 2)]
        elif(die1 == die2 or die2 == die3 or die1 == die3):
            return [round(132/216, 2), round(0.11574*2, 2), round(0.4444, 2), round(1/(6**3), 4)]
        return [-1,-1,-1,-1]
